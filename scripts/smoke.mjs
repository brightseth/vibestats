...
async function assertRoutes() {
  ...
  const revealDeleteScript = await readFile('scripts/delete-reveal-snapshot.mjs', 'utf8');
  const launchDoc = await readFile('docs/LAUNCH.md', 'utf8');
  const sharePlaybook = await readFile('docs/SHARE-PLAYBOOK.md', 'utf8');
  ...
  assert(packageJson.scripts?.['reveal:delete'] === 'node scripts/delete-reveal-snapshot.mjs', 'package should expose manual anonymous reveal link takedown');
  assert(revealDeleteScript.includes('delete from reveal_snapshots') && revealDeleteScript.includes('Usage: npm run reveal:delete -- <reveal-slug>'), 'anonymous reveal delete script should provide a bounded manual takedown path');
  ...
  assert(leaderboardHtml.includes('class="reveal-strip"') && leaderboardHtml.includes('Where do you rank?') && leaderboardHtml.includes('data-copy-command="/insights"') && leaderboardHtml.includes(NO_NPM_STATUS) && leaderboardHtml.includes('Copy status') && leaderboardHtml.includes(NO_NPM_REVEAL) && leaderboardHtml.includes(`data-copy-command="${NO_NPM_CLI}"`) && leaderboardHtml.includes(NO_NPM_INSTALL), 'leaderboard page should offer a direct no-npm terminal-first status, reveal, claim, and Claude Code install hook even when the board is populated');
  ...
  assert(matchHtml.includes('class="reveal-strip"') && matchHtml.includes('Find your real match') && matchHtml.includes('data-copy-command="/insights"') && matchHtml.includes(NO_NPM_STATUS) && matchHtml.includes('Copy status') && matchHtml.includes(NO_NPM_REVEAL) && matchHtml.includes(`data-copy-command="${NO_NPM_CLI}"`) && matchHtml.includes(NO_NPM_INSTALL), 'match page should offer a direct no-npm terminal-first status, reveal, claim, and Claude Code install hook even when matches are populated');
  ...
  assert(genomeHtml.includes('What are you?') && genomeHtml.includes('data-copy-command="/insights"') && genomeHtml.includes('Copy status preflight') && genomeHtml.includes('Copy reveal command') && genomeHtml.includes('Copy claim command') && genomeHtml.includes('profiles modeled') && genomeHtml.includes('Launch baseline; live submissions update this as people reveal.') && genomeHtml.includes(`data-copy-command="${NO_NPM_CLI}"`) && genomeHtml.includes(NO_NPM_STATUS) && genomeHtml.includes(NO_NPM_REVEAL) && genomeHtml.includes(NO_NPM_INSTALL) && genomeHtml.includes('Raw Claude Code /insights data stays local'), 'genome page should convert transparent community curiosity into terminal-first onboarding with claim and Claude Code install hooks');
  ...
  assert(recapHtml.includes('id="copy-sync"') && recapHtml.includes('Run CLI sync after more Claude Code work') && recapHtml.includes(`const SYNC_COMMAND = '${NO_NPM_CLI}';`), 'profile recap should let owners refresh the return surface with one-command no-npm terminal-first CLI sync');
  assert(recapHtml.includes('id="copy-install"') && recapHtml.includes(NO_NPM_INSTALL), 'profile recap should let owners install the Claude Code /vibestats return hook');
  assert(recapHtml.includes('id="recipient-reveal"') && recapHtml.includes('What are you?') && recapHtml.includes('Copy status') && recapHtml.includes('Copy reveal') && recapHtml.includes('Copy claim') && recapHtml.includes(`data-copy-command="${NO_NPM_STATUS}"`) && recapHtml.includes(`data-copy-command="${NO_NPM_CLI}"`) && recapHtml.includes(`data-copy-command="${NO_NPM_INSTALL}"`), 'profile recap should convert share recipients into no-npm terminal-first status, reveal, claim, and install');
  ...
  assert(cliBootstrapApi.includes('text/x-shellscript') && cliBootstrapApi.includes('codeload.github.com') && cliBootstrapApi.includes('( : < /dev/tty ) 2>/dev/null') && cliBootstrapApi.includes('node "$run_dir/bin/vibestats.js" "$@" < /dev/tty') && cliBootstrapApi.includes('Raw /insights data has not left this machine'), 'CLI bootstrap API should serve a no-npm local helper wrapper that preserves the privacy boundary and can prompt through curl-pipe stdin');
  ...
  assert(cliBin.includes('Minted GitHub-claimed, derived-only profile') && cliBin.includes('Privacy proof:') && cliBin.includes('Compare invite:') && cliBin.includes('Copy/paste:') && cliBin.includes('Share on X:') && cliBin.includes('Public discovery:') && cliBin.includes('Wrapped recap:') && cliBin.includes('Full launch kit:'), 'CLI sync success output should surface claimed identity, privacy proof, compare-first social share, opt-in discovery, recap, and a compact launch-kit handoff');
  ...
  assert(cliBin.includes('Install /vibestats for future reveals:') && cliBin.includes('Weekly digest:'), 'CLI sync success output should surface Claude Code and digest return hooks without dumping every secondary URL');
  ...
  assert(cliBin.includes('startBrowserClaimSession') && cliBin.includes('pollBrowserClaimSession') && cliBin.includes('vibestats profile claimed in browser'), 'CLI onboarding should race browser-side claiming against the terminal fallback');
  ...
  assert(syncTokenApi.includes("github:brightseth/vibestats#v0.1.0") && syncTokenApi.includes('VIBESTATS_CLI_PACKAGE'), 'sync token API should avoid the occupied unscoped npm package name while allowing package override');
  ...
  assert(statsApi.includes('statsSource(realTotal)') && statsApi.includes('launch-baseline-plus-live') && statsApi.includes('baselineStats()'), 'community stats API should label seeded baseline vs live submissions');
  assert(
  ...
  assert(indexHtml.includes('What kind of coder are you? Claude Code already knows.') && indexHtml.includes('<code>/insights</code>') && indexHtml.includes('Reveal on this machine') && indexHtml.includes('View sample reveal') && indexHtml.includes('Run the two steps below on your Claude Code machine') && indexHtml.includes('Copy reveal command') && indexHtml.includes(NO_NPM_CLI) && indexHtml.includes('Reveal locally, then choose whether to share') && indexHtml.includes('Anonymous share links are public unlisted'), 'upload page should frame onboarding as a two-step Claude Code reveal-first path with anonymous sharing up front');
  ...
  assert(indexHtml.includes('View sample reveal') && indexHtml.includes('Copy reveal command') && indexHtml.includes('On mobile, view the sample reveal now and send yourself the command for desktop later'), 'upload page should let cold visitors preview the reveal without competing with the real desktop command path');
  assert(indexHtml.includes('vibestatsPreview') && indexHtml.includes('this page only has derived preview metrics') && indexHtml.includes('Create anonymous share link') && indexHtml.includes('Copy preview URL') && indexHtml.includes('Public unlisted link; anyone with it can view') && indexHtml.includes('Claim with GitHub') && indexHtml.includes('GitHub claim unavailable') && indexHtml.includes('Claim this profile') && indexHtml.includes("fetch('/api/reveals'") && indexHtml.includes('if (!options.localPreview) submitStats') && !indexHtml.includes('Local terminal reveal'), 'upload page should render CLI-opened local previews with shareable hash URLs, anonymous share links, optional browser claim, and without publishing profile or community stats data');
  ...
  assert(indexHtml.includes('Have a legacy JSON export?') && !indexHtml.includes('Troubleshooting preflight'), 'upload page should demote legacy uploads and keep status checks out of the mainline onboarding path');
  ...
  assert(settingsHtml.includes(NO_NPM_STATUS) && settingsHtml.includes(NO_NPM_REVEAL) && settingsHtml.includes(`placeholder="${NO_NPM_CLI}"`) && settingsHtml.includes('terminal-first GitHub approval') && settingsHtml.includes('body.local_command || body.command'), 'settings UI should expose no-npm terminal-first CLI status, reveal, and one-command sync generation');
  ...
  assert(dashboardHtml.includes('/insights') && dashboardHtml.includes(NO_NPM_STATUS) && dashboardHtml.includes('Copy status') && dashboardHtml.includes(NO_NPM_REVEAL) && dashboardHtml.includes(`data-copy="${NO_NPM_CLI}"`) && dashboardHtml.includes(NO_NPM_INSTALL), 'static dashboard should teach share recipients the no-npm terminal-first status, reveal, claim, and Claude Code install hooks directly');
  ...
  assert(launchDoc.includes('Hard GitHub-claim gate: rotate any credential') && launchDoc.includes('npm run reveal:delete -- <reveal-slug>'), 'launch checklist should treat secret rotation as a GitHub-claim gate and document anonymous reveal takedown');
  assert(sharePlaybook.includes('Do not say "rare Deep Diver"') && sharePlaybook.includes('1-of-1 high-velocity Deep Diver combo') && sharePlaybook.includes('Lead with a link') && sharePlaybook.includes('Tag yourself'), 'share playbook should correct rarity copy and lead with mobile-first archetype sharing');
  ...
  assert(launchDoc.includes('npx --yes github:brightseth/vibestats#v0.1.0 share --handle <saved-gh-handle>') && launchDoc.includes('npm run share:kit -- --handle <saved-gh-handle>'), 'launch checklist should document terminal and maintainer first-profile share kits');
  ...
  assert(launchDoc.includes('VIBESTATS_CLI_PACKAGE') && launchDoc.includes('scoped package') && launchDoc.includes('Product-facing web onboarding should keep using the no-npm `/cli.sh` helper'), 'launch checklist should document the public npm package fallback switchover without replacing no-npm web onboarding');
  ...
  assert(launchDoc.includes('As of May 30, 2026') && launchDoc.includes('strict production identity audit passes') && launchDoc.includes('Preview auth testing still requires'), 'launch checklist should record current production readiness and preview auth caveat');
  ...
  assert(readme.includes('A successful sync mints a GitHub-claimed, derived-only profile') && readme.includes('profile, recap, privacy proof, compare invite, copy/paste share line, X share URL'), 'README should document CLI compare-first sync output');
  assert(readme.includes('versioned SSH shell contract') && readme.includes('/api/ssh/manifest') && readme.includes('curl -fsSL https://vibestats.io/cli.sh | sh -s -- claim VIBE-ABCD-2345') && readme.includes('npx --yes github:brightseth/vibestats#v0.1.0 claim VIBE-ABCD-2345') && readme.includes('`claim CODE` when an SSH/TUI session is waiting'), 'README should document the SSH shell contract and claim-session handoff commands');
  ...
  assert(readme.includes('VIBESTATS_CLI_PACKAGE') && readme.includes('GitHub release-tag fallback'), 'README should document the public CLI package override before broad npm sharing');
  assert(readme.includes('npx --yes github:brightseth/vibestats#v0.1.0 share --handle <saved-gh-handle>') && readme.includes('npm run share:kit -- --handle <saved-gh-handle>'), 'README should document terminal and maintainer copy-ready share kits for minted profiles');
  ...
  assert(readme.includes('privacy proof') && readme.includes('share --handle') && readme.includes('README badges, embeds'), 'README should document compact post-sync output and portable distribution snippets');
  ...
  assert(claudeCommand.includes(NO_NPM_STATUS) && claudeCommand.includes(NO_NPM_REVEAL) && claudeCommand.includes(`${NO_NPM_CLI} sync`) && !claudeCommand.includes('github:brightseth/vibestats#v0.1.0 join') && claudeCommand.includes('Only after the user agrees'), 'Claude Code command should preflight, reveal locally, then explicit no-npm sync after human consent');
  ...
  const results = await auditPackage(parsed, {
    exec: async (bin, args) => {
      ...
      if (bin === 'npm' && args.includes('--help')) {
        return {
          stdout: [
            'Current no-npm status command: curl -fsSL https://vibestats.io/cli.sh | sh -s -- status',
            'Current npx fallback command: npx --yes @lets-vibe/vibestats',
      ...
      if (bin === 'npm' && args.includes('reveal')) {
        ...
        err.stdout = [
          ...
          '2. Check terminal readiness: curl -fsSL https://vibestats.io/cli.sh | sh -s -- status',
        ].join('\n');
        err.stderr = '';
        throw err;
      }
...
async function assertUpdateCliCommandScript() {
  ...
  const readme = report.files.find((item) => item.file === 'README.md');
  ...
  assert(report.replacements > 0 && readme?.count > 0 && !TARGET_FILES.includes('home.html'), 'CLI command update script should still cover npx fallback docs while public static snippets stay on the no-npm helper');
  assert(TARGET_FILES.includes('README.md') && TARGET_FILES.includes('settings.html') && TARGET_FILES.includes('.claude/commands/vibestats.md'), 'CLI command update script should keep scanning docs, settings, and Claude Code install command for stale npx fallback snippets');
...
async function assertDerivedProfileCredentialHelpers() {
  ...
  assert(credential.method.spec_url === 'https://vibestats.example/api/derived-profile-spec' && credential.method.spec_path === '/api/derived-profile-spec' && credential.method.source.id === 'claude_code_insights' && credential.method.future_source_ready === true, 'derived credential should link the source-agnostic public spec and current local source evidence');
    ...
    const bodyFor = () => {
      if (parsed.pathname === '/api/ssh/claim-start') {
        ...
        return {
          ...
          npx_command: "npx --yes 'github:brightseth/vibestats#v0.1.0' claim 'VIBE-ABCD-2345' --host 'https://vibestats.example'",
        };
      }
...
async function assertCliBootstrapScript() {
  ...
  assert(githubTarballUrl() === 'https://codeload.github.com/brightseth/vibestats/tar.gz/refs/tags/v0.1.0', 'CLI bootstrap should default to the pinned GitHub release tag tarball');
  ...
  assert(script.includes('( : < /dev/tty ) 2>/dev/null') && script.includes('node "$run_dir/bin/vibestats.js" "$@" < /dev/tty') && script.includes('else\n  node "$run_dir/bin/vibestats.js" "$@"'), 'CLI bootstrap should execute the repo-local CLI with forwarded args and restore TTY stdin for prompts after curl-pipe launch only when a controlling TTY is available');
...
async function assertProfileShareLoop() {
  ...
  assert(profileHtml.includes('Claude Code has already captured your build fingerprint') && profileHtml.includes('STATUS_COMMAND') && profileHtml.includes(NO_NPM_STATUS) && profileHtml.includes(NO_NPM_REVEAL), 'profile reveal panel should carry the no-npm terminal-first status and reveal command path without sending visitors hunting');
  assert(profileHtml.includes('CLAIM_COMMAND') && profileHtml.includes('Copy claim') && profileHtml.includes('Copy status') && profileHtml.includes('INSTALL_CLAUDE_COMMAND') && profileHtml.includes(NO_NPM_INSTALL), 'profile reveal surfaces should let share recipients preflight, claim from terminal, and install the Claude Code command');
...
async function assertWrappedShareLoop() {
  ...
  assert(wrappedHtml.includes('What are you?') && wrappedHtml.includes('/insights') && wrappedHtml.includes(NO_NPM_STATUS) && wrappedHtml.includes('Copy status') && wrappedHtml.includes(NO_NPM_REVEAL) && wrappedHtml.includes(`data-copy="${NO_NPM_CLI}"`) && wrappedHtml.includes(NO_NPM_INSTALL), 'wrapped share page should teach recipients the no-npm terminal-first status, reveal, claim, and Claude Code install hooks directly');
...
async function assertCliDerivedPayload() {
  ...
  assert(DEFAULT_NPX_SYNC_COMMAND === 'npx --yes github:brightseth/vibestats#v0.1.0', 'CLI should expose the current GitHub-backed npx command');
  assert(DEFAULT_NPX_REVEAL_COMMAND === 'npx --yes github:brightseth/vibestats#v0.1.0 reveal', 'CLI should expose the current terminal-first reveal command');
  assert(DEFAULT_NPX_STATUS_COMMAND === 'npx --yes github:brightseth/vibestats#v0.1.0 status', 'CLI should expose the current terminal readiness preflight command');
  assert(DEFAULT_NPX_JOIN_COMMAND === 'npx --yes github:brightseth/vibestats#v0.1.0 join', 'CLI should expose the current terminal-first join command');
  assert(DEFAULT_INSTALL_COMMAND === 'npx --yes github:brightseth/vibestats#v0.1.0 install-claude-command', 'CLI should expose a copyable Claude Code command installer');
  ...
  assert(cliSource.includes('Current no-npm claim command: ${DEFAULT_LOCAL_SYNC_COMMAND}') && cliSource.includes('Current no-npm reveal command: ${DEFAULT_LOCAL_REVEAL_COMMAND}') && cliSource.includes('Current npx fallback command: ${DEFAULT_NPX_SYNC_COMMAND}') && cliSource.includes('Use --dry-run as a legacy alias for reveal') && cliSource.includes('Use reveal --json to print the exact derived payload'), 'CLI help should separate no-npm claim, human reveal, npx fallback, and payload JSON');
  const missingAdvice = cliErrorMessage(new Error(`No Claude Code /insights session metadata found in ${join('~', '.claude', 'usage-data')}.`));
  assert(missingAdvice.includes('Terminal onboarding:') && missingAdvice.includes('/insights') && missingAdvice.includes(NO_NPM_STATUS) && missingAdvice.includes(NO_NPM_REVEAL) && missingAdvice.includes(NO_NPM_CLI), 'CLI missing-insights errors should recover into a no-npm terminal onboarding checklist');
  ...
  const webPreviewUrl = localWebPreviewUrl(insights, { host: 'https://vibestats.example', claimCode: 'VIBE-ABCD-2345' });
  ...
  try {
    ...
    assert(readyStatusText.includes('Status: ready for reveal') && readyStatusText.includes(DEFAULT_LOCAL_REVEAL_COMMAND) && readyStatusText.includes(DEFAULT_LOCAL_SYNC_COMMAND), 'CLI status text should hand ready users into no-npm reveal and claim commands');
    ...
    assert(missingStatus.ready === false && missingStatus.next_steps.some((step) => step.includes('/insights')) && missingStatus.status_command === DEFAULT_LOCAL_STATUS_COMMAND, 'CLI status JSON should recover missing users into the /insights no-npm terminal checklist');
    ...
    assert(shareKitJson.copy.readme_badge_markdown.includes('/u/alex/badge.svg') && shareKitJson.copy.terminal_onboarding.includes(localHelperCommand('install-claude-command', { host: 'https://vibestats.example' })), 'CLI share JSON should expose portable distribution snippets and the no-npm Claude Code install hook');
    ...
    const fallbackAuth = await fallbackPromise;
	    ...
	    const startedClaim = await startBrowserClaimSession({
	      host: 'https://vibestats.example',
	      fetchImpl: async (url, options = {}) => {
	        assert(url === 'https://vibestats.example/api/ssh/claim-start' && options.method === 'POST', 'browser claim race should start a bounded server-side claim session');
	        return {
	          ok: true,
	          async json() {
	            return {
	              code: 'VIBE-ABCD-2345',
	              status_url: 'https://vibestats.example/api/ssh/claim-status?code=VIBE-ABCD-2345',
	              expires_at: '2026-06-01T00:00:00.000Z',
	            };
	          },
	        };
	    ...
	    assert(startedClaim.code === 'VIBE-ABCD-2345', 'browser claim race should expose the sanitized claim code for the web-preview hash');
	    let pollAttempts = 0;
	    const polledClaim = await pollBrowserClaimSession({
	      host: 'https://vibestats.example',
	      code: startedClaim.code,
	      timeoutMs: 1000,
	      intervalMs: 1,
	      sleepImpl: async () => {},
	      fetchImpl: async (url) => {
	        pollAttempts += 1;
	        assert(String(url).includes('/api/ssh/claim-status?code=VIBE-ABCD-2345'), 'browser claim race should poll the no-store claim status endpoint');
	        return {
	          ok: true,
	          async json() {
	            return pollAttempts === 1
	              ? { ok: true, state: 'pending' }
	              : { ok: true, state: 'synced', gh_handle: 'alex', profile_url: '/u/alex', compare_url: '/?compareTo=alex&compareArchetype=shipper', recap_url: '/u/alex/recap', credential_url: '/u/alex/credential.json' };
	          },
	        };
	      },
	    });
	    assert(polledClaim.state === 'synced' && polledClaim.gh_handle === 'alex', 'browser claim race should resolve when the browser save consumes the claim session');

	    output.length = 0;
	    const browserRaceOpenedUrls = [];
	    const browserRaceResult = await sync({
	      file,
	      host: 'https://vibestats.example',
	      dryRun: false,
	      promptToPublish: true,
	      open(url) {
	        browserRaceOpenedUrls.push(url);
	        return true;
	      },
	      startBrowserClaimSession: async () => ({ code: 'VIBE-ABCD-2345' }),
	      pollBrowserClaimSession: async () => ({
	        state: 'synced',
	        gh_handle: 'alex',
	        profile_url: '/u/alex',
	    ...
	    const browserPreviewPayload = JSON.parse(Buffer.from(new URL(browserRaceOpenedUrls[0]).hash.replace(/^#vibestatsPreview=/, ''), 'base64url').toString('utf8'));
	    assert(browserRaceResult.via === 'browser' && browserRaceResult.profile_url === 'https://vibestats.example/u/alex', 'CLI onboarding should exit successfully when the browser claim wins the race');
	    assert(browserPreviewPayload.claim_code === 'VIBE-ABCD-2345', 'CLI onboarding should embed the browser-claim race code in the fragment-only web preview');
	    assert(output.join('').includes('Waiting for browser claim to finish') && output.join('').includes('vibestats profile claimed in browser'), 'CLI onboarding should wait for browser claim completion before offering terminal fallback');
	    assert(output.join('').includes('Raw Claude Code /insights data stayed local. The browser published only derived metrics.'), 'CLI browser-claim race should keep the privacy proof in terminal output');
    ...
    assert(output.join('').includes('Compare invite: https://vibestats.example/?compareTo=alex&compareArchetype=shipper'), 'CLI sync should print compare-first invite URL');
    ...
    assert(output.join('').includes('Privacy proof: https://vibestats.example/u/alex/credential.json'), 'CLI sync should print the machine-readable derived credential proof URL');
    assert(output.join('').includes("Copy/paste: I just claimed my Claude Code build profile: prolific Shipper. Raw /insights stayed local. What are you? See how you'd pair with me: https://vibestats.example/?compareTo=alex&compareArchetype=shipper"), 'CLI sync should print a copy-ready privacy-aware compare-first share line');
    ...
    assert(output.join('').includes('Public discovery: https://vibestats.example/settings#privacy-settings'), 'CLI sync should print the opt-in public discovery settings URL');
    assert(output.join('').includes('Match intent: https://vibestats.example/settings#match-settings'), 'CLI sync should print the match intent setup URL');
    assert(output.join('').includes('Weekly board: https://vibestats.example/leaderboard/shipper'), 'CLI sync should print the archetype leaderboard return URL');
    assert(output.join('').includes('Matchmaker: https://vibestats.example/match?goal=pair-coding&archetype=shipper'), 'CLI sync should print the matchmaker return URL');
    assert(output.join('').includes('Wrapped recap: https://vibestats.example/u/alex/recap'), 'CLI sync should print recap return URL');
    ...
    assert(output.join('').includes(`Full launch kit: ${localHelperCommand('share --handle alex', { host: 'https://vibestats.example' })}`), 'CLI sync should hand secondary badge and embed output to the share-kit command');
    ...
    assert(output.join('').includes('Weekly digest: https://vibestats.example/settings#weekly-digest-row'), 'CLI sync should print the weekly digest setup link as a return hook');
