...
async function assertRoutes() {
  ...
  assert(settingsHtml.includes('npx --yes github:brightseth/vibestats#feat/wave-1-identity join') && settingsHtml.includes('terminal-first GitHub approval'), 'settings UI should expose terminal-first CLI sync command generation');
  ...
  assert(readme.includes('Terminal-first onboarding is intentionally short') && readme.includes('`join` and `onboard` are aliases') && readme.includes('No website upload is required'), 'README should document terminal-first CLI onboarding aliases without manual website upload');
...
async function assertCliDerivedPayload() {
  ...
  assert(revealText.includes('For machine-readable derived payload: add --json.'), 'CLI dry-run reveal should point auditors to JSON mode');
