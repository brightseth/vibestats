...
async function assertRoutes() {
  ...
  assert(cliBootstrapApi.includes('text/x-shellscript') && cliBootstrapApi.includes('codeload.github.com') && cliBootstrapApi.includes('node "$run_dir/bin/vibestats.js" "$@" < /dev/tty') && cliBootstrapApi.includes('Raw /insights data has not left this machine'), 'CLI bootstrap API should serve a no-npm local helper wrapper that preserves the privacy boundary and can prompt through curl-pipe stdin');
...
async function assertCliBootstrapScript() {
  ...
  assert(script.includes('node "$run_dir/bin/vibestats.js" "$@" < /dev/tty') && script.includes('else\n  node "$run_dir/bin/vibestats.js" "$@"'), 'CLI bootstrap should execute the repo-local CLI with forwarded args and restore TTY stdin for prompts after curl-pipe launch');
