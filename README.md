...
After publishing, set `VIBESTATS_CLI_PACKAGE=@lets-vibe/vibestats` so npm fallback commands generated by Settings and CLI follow-up output print the public package spec instead of the GitHub branch fallback. The product-facing web snippets should stay on the no-npm `/cli.sh` helper.
