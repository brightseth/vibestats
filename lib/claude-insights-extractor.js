...
import { aggregateFacetSignals } from '../api/_lib/facet-signals.js';
...
export function insightsFromClaudeUsage({ sessions = [], facets = [], reportDateRange = '' } = {}) {
  ...
  for (const session of sessions) {
    ...
    longestSessionMinutes = Math.max(longestSessionMinutes, number(session.duration_minutes));
    filesModified += number(session.files_modified);
    linesChanged += number(session.lines_added) + number(session.lines_removed);
  ...
  // Depth layer: counts-only distributions from the rich facet files. Local only;
  // free text is never read. See api/_lib/facet-signals.js for the privacy contract.
  const facetSignals = aggregateFacetSignals(facets);
