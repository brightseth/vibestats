...
export function derivedUploadPayloadFromInsights(data = {}, { source = 'cli' } = {}) {
  ...
  // Counts-only depth signals from the facet layer (already aggregated locally by
  // the extractor). Server re-validates against the allowlist on ingest.
  const facetSignals = sanitizeFacetSignals(data.metrics?.facet_signals);
